namespace TestSuite
{
    public class ShortestRemainingTimeFirstTest
    {
        IEnumerable<Processo> Processos { get; set; }
        Escalonador Escalonador { get; set; }

        public ShortestRemainingTimeFirstTest()
        {
            Processos = GeradorProcessos.GeraProcessosPadroes();
            Escalonador = new ShortestRemainingTimeFirst(new ReadyQueue(Processos)); 
            Escalonador.Executa();
        }
        ...
        public void RightConclusionTimes()
        {
            ...
            Assert.True(Processos.ElementAt(2).TempoTermino == 7);
